...
import requests
import warnings
from .config import SELECTED_WEATHER_VARS, TIME_FEATURES, ARCHIVE_URL, FORECAST_URL, MAX_PAST_DAYS_FORECAST, CHUNK_LENGTH_FORECAST
...
# ────────────────────────────────────────────────────────────────
# simple disk-cache helper  (cache/elec_combined_YYYYMMDD_YYYYMMDD.pkl)
# ────────────────────────────────────────────────────────────────
CACHE_DIR = Path(__file__).resolve().parent / "cache"
CACHE_DIR.mkdir(exist_ok=True)


def _combined_cache_path(start_date: datetime, end_date: datetime) -> Path:
    key = f"elec_combined_{start_date:%Y%m%d}_{end_date:%Y%m%d}.pkl"
...
def load_weather_data(
    ...
    """Return a DataFrame with the requested weather parameters.

    • Older than MAX_PAST_DAYS_FORECAST → /archive
    • Newer / future data             → /forecast
    """
    ...
    today_utc = datetime.utcnow().replace(hour=0, minute=0, second=0,
                                          microsecond=0)
    oldest_forecast_ok = today_utc - timedelta(days=MAX_PAST_DAYS_FORECAST)

    # Build a list of chunks we have to download
    chunks = []
    ...
    if recent_start <= end_date:
        cur = recent_start
        while cur <= end_date:
            cur_end = min(cur + timedelta(days=CHUNK_LENGTH_FORECAST - 1),
                          end_date)
    ...
    for kind, s, e in chunks:
        ...
        try:
            ...
            raise RuntimeError(f"[Open-Meteo] {kind} "
                               f"{s.date()}–{e.date()} failed: {ex}") from ex
    ...
    if "wind_speed_10m" in df_all.columns:
        ...
        df_all["wind_speed_10m"] /= 3.6
    if "shortwave_radiation" in df_all.columns:
        # W/m² to kWh/m²  (hourly means, 1 W = 1 J/s)
        df_all["shortwave_radiation"] /= 1000
    ...
    print(f"[Open-Meteo] got {weather.shape} rows "
...
def load_electricity_data_from_api(energy_type, start_date=None, end_date=None):
    ...
    # ───── 1️⃣  try to serve cached combined data  ───────────────
    if energy_type == "combined":
        cache_file = _combined_cache_path(start_date, end_date)
        if cache_file.exists():
            print(
                f"[cache] combined electricity  {start_date.date()}–{end_date.date()} loaded")
            # columns dict unused
            return pd.read_pickle(cache_file), {}

    # ───── 2️⃣  otherwise fetch from CodeGreen  ─────────────────
    country_code = "DE"
    ...
    if not isinstance(df.index, pd.DatetimeIndex):
        ts_col = next((c for c in ["startTime", "startTimeUTC", "time",
                                   "timestamp", "datetime"] if c in df.columns), None)
        if ts_col:
    ...
    if energy_type == "combined":
        required_cols = {"percentRenewableWS", "total"}
        if required_cols.issubset(df.columns):
            electricity_data["percentRenewableWS"] = df["percentRenewableWS"] / 100.0
            #  Save to disk so later runs reuse it
            electricity_data.to_pickle(cache_file)
            print(f"[cache] combined electricity saved → {cache_file.name}")
        else:
            raise ValueError(
                f"Columns {required_cols} missing in API response")
    ...
    if CODEGREEN_AVAILABLE:
        try:
            electricity_data, _ = load_electricity_data_from_api(
                energy_type, start_date, end_date)
            if electricity_data.index.tz is None:
                electricity_data.index = electricity_data.index.tz_localize(
                    'Europe/Berlin')
        ...
        raise ImportError(
            "codegreen_core package is not available.")
...
def preprocess_data(weather_data, electricity_data, selected_params=None):
    ...
    # Convert timezone-aware indices to UTC and make naive; leave naive indices unchanged
    ...
    # Verify both are now naive
    assert weather_data.index.tz is None, "Weather data index is still tz-aware"
    assert electricity_data.index.tz is None, "Electricity data index is still tz-aware"

    # Log date ranges after conversion
    ...
    # Proceed with date range comparison
    ...
    if start_date >= end_date:
        ...
        print(
            f"Weather data: {weather_data.index.min()} to {weather_data.index.max()}")
        print(
            f"Electricity data: {electricity_data.index.min()} to {electricity_data.index.max()}")
        raise ValueError(
...
def fetch_and_prepare(start: datetime, end: datetime):
    """
    100 % identical to the code in exp2.py.
    ...
    elec_raw = load_electricity_data(None, "combined", start, end)

    if {"renewableTotalWS", "total"}.issubset(elec_raw.columns):
        elec_raw["y"] = elec_raw["renewableTotalWS"]
    elif "percentRenewableWS" in elec_raw.columns:
        elec_raw = elec_raw.rename(columns={"percentRenewableWS": "y"})
    else:
        raise RuntimeError("Could not build renewable-percentage target")

    df, _ = preprocess_data(weather, elec_raw,
...
def build_sequences(df, lookback, horizon):
    vals  = df.values.astype(np.float32)
    n     = len(vals)
    feats = df.shape[1] - 1         # last col is target
    X, y, t = [], [], []
    for i in range(lookback, n - horizon + 1):
        X.append(vals[i-lookback:i, :feats])
