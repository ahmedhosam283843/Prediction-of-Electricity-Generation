...
def plot_predictions(predictions, targets, times,
    ...
    """
    Draws the first <num_points> predictions vs. ground-truth for a given horizon
    (1-based; e.g. horizon=72 means the last column).
    """
    if predictions.ndim == 1:  # single-step case
    ...
    if h >= forecast_horizon:
        raise ValueError(f"Horizon {horizon} exceeds available outputs")
    ...
    ax.plot(times_h, target_h, label='Actual',  lw=1, alpha=.7)
    ax.plot(times_h, pred_h,  label=f'{horizon}-h Forecast', lw=1, ls='--', alpha=.7)
    ax.set_xlabel('Time'), ax.set_ylabel('% Renewable')
    ax.set_title(f'{model_name}: {horizon}-h Ahead Forecast')
